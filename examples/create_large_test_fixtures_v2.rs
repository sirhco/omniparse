//! Create large test fixtures for performance testing using calamine
//! 
//! This version uses calamine to write the files to ensure compatibility

use calamine::{Xlsx, DataType};
use std::fs::File;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // For now, let's just create the PPTX file since that's easier
    // and use a simpler approach for XLSX
    create_large_pptx()?;
    create_simple_large_xlsx()?;
    
    println!("All large test fixtures created successfully!");
    Ok(())
}

fn create_simple_large_xlsx() -> Result<(), Box<dyn std::error::Error>> {
    println!("Creating simple_large_sample.xlsx with 10,000 cells...");
    
    let file = File::create("test_data/document/simple_large_sample.xlsx")?;
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>
    ...
    let mut worksheet = String::from(r#"<?xml version="1.0" encoding="UTF-8"?>
...
<sheetData>"#);
    ...
    zip.finish()?;
    println!("Created simple_large_sample.xlsx with 10,000 cells");
    Ok(())
}
